  ...
  const start = async (competition: Icompetition) => {
    ...
    if (res.data.msg == "success") {
      ...
      win.Swal.fire({
        ...
        text:
          res.data.msg ||
          "you can't activate this competition please contact admin.",
      });
    }
  };
        ...
        if (alert) {
          competitions.forEach((comp, i) => {
            if (competition.id == comp.id)
              return (competitions[i].status = "finished");
          });
        }
      }
    } else {
...
>
  ...
  >
    <div class="offcanvas-header">
      <h5 class="offcanvas-title" id="competitionCanvasLabel">
        Add Competition
      </h5>
      <button
        type="button"
